...
def JsonFileToDict(JsonPath):
	if type(JsonPath) == str and len(JsonPath) > 0:
		with open(JsonPath, 'r', encoding="utf-8") as file:
			return json.load(file)
	else:
		raise TypeError("JsonPath precisa ser uma String com pelo menos 1 caractere!")

#Converte um arquivo JSON em um arquivo YAML (retorna um novo YamlFile)
def JsonFileToYamlFile(JsonPath, YamlPath, save=False):
	if type(JsonPath) == str and len(JsonPath) > 0 and type(YamlPath) == str and len(YamlPath) > 0 and type(save) == bool:
		dicionario = JsonFileToDict(JsonPath)
		...
		raise TypeError("JsonPath e YamlPath precisam ser Strings com pelo menos 1 caractere e/ou save tem que ser um Booleano!")

#Converte um arquivo YAML para um dicionario
def YamlFileToDict(YamlPath):
	if type(YamlPath) == str and len(YamlPath) > 0:
		return YamlFile(YamlPath).data
	else:
		raise TypeError("YamlPath precisa ser uma String com pelo menos 1 caractere!")

#Converte um arquivo YAML para um arquivo JSON
def YamlFileToJsonFile(YamlPath, JsonPath):
	if type(YamlPath) == str and len(YamlPath) > 0 and type(JsonPath) == str and len(JsonPath) > 0:
		dicionario = YamlFileToDict(YamlPath)

		with open(JsonPath, 'w', encoding="utf-8") as file:
			file.write(json.dumps(dicionario, ensure_ascii=False, indent="\t"))
	else:
		raise TypeError("YamlPath e JsonPath precisam ser Strings com pelo menos 1 caractere!")
